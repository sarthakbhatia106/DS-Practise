...
public class one {
    ...
    public static class p implements Comparable<p>{
        int diff;
        int idx;
        public p(int diff,int idx){
            this.diff=diff;
            this.idx=idx;
        }
        @Override
        public int compareTo(one.p o) {
    ...
    static void solve() {
        ...
        for(int i=0;i<n;i++){
            sum+=(a[i]-b[i]);
            if(a[i]-b[i]!=0){
                arr.add(new p(a[i]-b[i],i));
            }
        }
        Collections.sort(arr);
        // for(p i:arr){
        //     System.out.print(i.diff+" "+i.idx+"-");
        // }
        ...
        if(sum!=0){
            ...
            while(left<right){
                ...
                while(arr.get(left).diff<0 && arr.get(right).diff>0){
                    int []r=new int[2];
                    r[0]=arr.get(right).idx+1;
                    r[1]=arr.get(left).idx+1;
                    al.add(r);
                    arr.get(left).diff++;
                    arr.get(right).diff--;
                    // System.out.println(arr.get(left).diff+" "+arr.get(right).diff);
                }
                if(arr.get(left).diff==0){
                    left++;
                }

                if(arr.get(right).diff==0){
                    right--;
                }
            }
