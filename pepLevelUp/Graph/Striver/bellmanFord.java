...
public class bellmanFord {
    ...
    private static void shortestDist(ArrayList<node> graph, int n) {
        int[] dist = new int[n];
        ...
        for (int i = 0; i < n - 1; i++) {
            for (node nbr : graph) {
                if (dist[nbr.u] != Integer.MAX_VALUE && dist[nbr.u] + nbr.wt < dist[nbr.v]) {
                    dist[nbr.v] = dist[nbr.u] + nbr.wt;
                }
            }
        }
        ...
        if (flag == false) {
            System.out.println("Shortest distance to all nodes :- ");
            for (int i : dist) {
                System.out.print(i + " ");
            }
            System.out.println();
