...
public class shortestPathDAG {
    ...
    static void solve() {
        ...
        int[] dist = shortestPath(src, graph, topo);

        for (int i = 0; i < dist.length; i++) {
            sb.append((dist[i]==Integer.MAX_VALUE?"Not Reachable":dist[i])+" ");
        }
        sb.append("\n");
    }

    private static int[] shortestPath(int src, ArrayList<ArrayList<node>> graph, int[] topo) {
        int n = graph.size();
        int[] dist = new int[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (dist[node] != Integer.MAX_VALUE) {
                for (node nbr : graph.get(node)) {
                    if (dist[node] + nbr.wt < dist[nbr.v]) {
    ...
    private static int[] topoSort(ArrayList<ArrayList<node>> graph) {
