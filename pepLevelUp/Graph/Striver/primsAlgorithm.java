...
public class primsAlgorithm {
    ...
    private static void primsAlgo(ArrayList<ArrayList<node>> graph) {
        int n=graph.size();
        int []key=new int[n];
        ...
        for(int i=0;i<n-1;i++){
            ...
            for(node a: graph.get(u)){
                if(mst[a.v]==false && a.wt<key[a.v]){
                    parent[a.v]=u;
                    key[a.v]=a.wt;
                    pq.add(new node(a.v,key[a.v]));
                }
            }
        }
        for(int i=1;i<n;i++){
    ...
    private static void primsAlgoBrute(ArrayList<ArrayList<node>> graph) {
        int n=graph.size();
        int []key=new int[n];
        int []parent=new int[n];
        boolean []mst=new boolean[n];
