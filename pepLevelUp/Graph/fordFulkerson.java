...
public class fordFulkerson {
    ...
    static int solve(int N, int M, ArrayList<ArrayList<Integer>> Edges) {
        // adjacency matrix will be beneficial in this case as back edges are to be made
        // and editted
        ...
        while (bfs(graph, src, sink, parent)) {
            ...
            for (int i = sink; i != src; i = parent[i]) {
                min=Math.min(graph[parent[i]][i], min);
            }

            for (int i = sink; i != src; i = parent[i]) {
                graph[parent[i]][i]-=min;
                graph[i][parent[i]]+=min;
            }
            ans+=min;
        }
        return ans;
    ...
    private static boolean bfs(int[][] graph, int src, int sink, int[] parent) {
