...
public class fenwickTree {
	...
	static void solve() throws IOException{
        ...
        long []farr=new long[n+1];

        for(int i=0;i<n;i++){
            update(farr,i+1,arr[i]);
        }

        for(int i=0;i<q;i++){
            if(ch[i]=='q'){
                long val=(get(farr,query[i][1])-get(farr, query[i][0]-1));
                sb.append(val+"\n");
            }else{
                update(farr, query[i][0], query[i][1]);
            }
        }

	}
    private static long get(long[] farr, int i) {
        long ans=0;
        while(i>0){
            ans+=farr[i];
            i=i-(i&-i);
    ...
    private static void update(long[] farr, int i, int j) {
        while(i<farr.length){
            farr[i]+=j;
            i=i+(i&-i);
        }
