...
public class palindromeSubsequence {
    ...
    static void solve() {
        String str = s();
        sb.append(countOfPalindrome(str)+"\n");
    }

    private static int countOfPalindrome(String str) {
        int n=str.length();
        int [][]dp=new int[n][n];
        ...
        for(int gap=0;gap<n;gap++){
            for(int left=0;left<n-gap;left++){
                ...
                if(str.charAt(left)==str.charAt(right)){
                    dp[left][right]=(left+1<n?dp[left+1][right]:0)+(right-1>=0?dp[left][right-1]:0)+1;
                }else{
                    dp[left][right]=(left+1<n?dp[left+1][right]:0)+(right-1>=0?dp[left][right-1]:0);
