...
public class optimalBST {
    ...
    static void solve() {
        int n = i();
        int[] key = readArray(n);
        int[] freq = readArray(n);

        int[] preFreq = new int[n];
        preFreq[0] = freq[0];
        for (int i = 1; i < preFreq.length; i++) {
            preFreq[i] = preFreq[i - 1] + freq[i];
        }
        sb.append(optimalBst(key, freq, preFreq));
    }

    private static int optimalBst(int[] key, int[] freq, int[] preFreq) {
        int n = key.length;
        int[][] dp = new int[n][n];
        ...
        for (int gap = 1; gap < n; gap++) {
            for (int left = 0; left < n - gap; left++) {
                ...
                for (int i = left; i <= right; i++) {
                    int val = (i-1 >= 0 ? dp[left][i - 1] : 0) + (i+1 < n ? dp[i + 1][right] : 0) + preFreq[right]
                            - (left - 1 < 0 ? 0 : preFreq[left - 1]);
                    dp[left][right] = Math.min(dp[left][right], val);
                }
            }
        }
        return dp[0][n - 1];
