    ...
    private static int mcm(int[] arr) {
        ...
        for(int gap=1;gap<dp.length;gap++){
            for(int left=0;left<dp.length-gap;left++){
                ...
                if(gap==1){
                    dp[left][right]= arr[left]*arr[right]*arr[right+1];
                }else{
                    for(int i=left;i<right;i++){
                        //left se i is left half, i+1 se right tak is right half
                        int lc=dp[left][i];
                        int rc=dp[i+1][right];

                        //dimensions in the real array will always be left half-> left,k+1 and right half->k+1,right+1
                        int mc=arr[left]*arr[i+1]*arr[right+1];
                        dp[left][right]=Math.min(dp[left][right], lc+rc+mc);
                    }
                }
            }
        }
