    ...
    static void solve() {
        ...
        for(int gap=0;gap<n;gap++){
            for(int left=0;left<n-gap;left++){
                ...
                int val1=(left+2<=right)?dp[left+2][right]:0;
                int val2=(left+1<=right-1)?dp[left+1][right-1]:0;
                int val3=(right-2>=left)?dp[left][right-2]:0;

                int ans1=arr[left]+Math.min(val1,val2);
                int ans2=arr[right]+Math.min(val2,val3);
