...
public class cherryPickupProblem2 {
    ...
    public static int cherryPickup(int[][] grid) {
        int n = grid.length;
        int m = grid[0].length;
        int ans = cherryDP(grid, 0, 0, m - 1, new Integer[n][m][m]);
        return ans == Integer.MIN_VALUE ? 0 : ans;
    }

    public static int cherryDP(int[][] arr, int r1, int c1, int c2, Integer[][][] dp) {
        if (r1 >= arr.length || c1 >= arr[0].length || c2 >= arr[0].length || c1<0 || c2<0) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                int temp = cherryDP(arr, r1Dash, c1Dash, c2Dash, dp);
