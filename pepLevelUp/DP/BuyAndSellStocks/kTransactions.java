...
public class kTransactions {
    ...
    static void solve() {
        ...
        sb.append(statesSolution(k,arr)+"\n");
    }

    private static int normal(int[] arr, int n,int k) {
        ...
        for(int i=1;i<=k;i++){
            for(int j=1;j<n;j++){
                ...
                for(int prev=0;prev<j;prev++){
                    max=Math.max(dp[i-1][prev]+arr[j]-arr[prev],max);
                }
                dp[i][j]=max;
            }
        }
    ...
    private static int normalOptimised(int[] arr, int n,int k) {
        int [][]dp=new int[k+1][n];
        ...
        for(int i=1;i<=k;i++){
            ...
            for(int j=1;j<n;j++){
                ...
                dp[i][j]=Math.max(dp[i-1][j],max+arr[j]);
            }
        }
        return dp[k][n-1];
    }
    public static int statesSolution(int k, int[] prices) {
