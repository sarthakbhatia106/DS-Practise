...
public class cooldown {
    ...
    static void solve() {
        ...
        sb.append(statesSolution(arr)+"\n");
    }

    private static int normal(int[] arr, int n) {
        int bp=-arr[0];
        int sp=0;
        int cp=0;
        ...
        for (int i = 1; i < arr.length; i++) {
            int temp=bp;
            int temp1=sp;
            bp=Math.max(bp,cp-arr[i]);
            sp=Math.max(sp,temp+arr[i]);
            cp=Math.max(cp,temp1);
        }
        return sp;
    }
    public static int statesSolution(int[] prices) {
        ...
        for(int val:prices){
            ...
            dpi1=Math.max(dpi1,dpi0Cool-val);
            dpi0Cool=temp;
        }
        return dpi0;
    }

    public static int statesSolutionVariableCoolDown(int[] prices) {
        
        int dpi0=0;
        Queue<Integer> q=new ArrayDeque<>();
        //add same number of zeroes as the cooldown period. In this case cooldown is 1
        q.add(0);
        int dpi1=Integer.MIN_VALUE;
        ...
        for(int val:prices){
            int temp=dpi0;
            dpi0=Math.max(dpi0,dpi1+val);
            dpi1=Math.max(dpi1,q.remove()-val);
            q.add(temp);
        }
        return dpi0;
    }
