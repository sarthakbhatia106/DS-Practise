...
public class EggBreakingProblem {
    ...
    static void solve() {
        int eggs=i();
        int floors=i();
        sb.append(eggDropFE(eggs,floors)+"\n");
    }
    

    private static int eggDropFSquareE(int eggs, int floors) {
        ...
        for(int i=2;i<=eggs;i++){
            for(int j=2;j<=floors;j++){
                ...
                for(int k=1;k<=j;k++){
                    int ans=1+Math.max(dp[i][j-k], dp[i-1][k-1]);
    ...
    public static int eggDropFE(int eggs,int floors){
