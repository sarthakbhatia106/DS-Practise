...
public class classyNumber {
    ...
    static void solve() {
        ...
        s = r + "";
        long ans2 = solver(0, 0, true);
        sb.append(ans2 - ans1 + "\n");
    }

    private static long solver(int pos, int count, boolean bound) {
        if (pos == s.length()) {
            return 1;
        }
        if (dp[pos][count][(bound == true) ? 1 : 0] != -1) {
            return dp[pos][count][(bound == true) ? 1 : 0];
        }
        long ans = 0;
        long maxDigit = -1;
        if (bound == true) {
        ...
        for (int i = 0; i <= maxDigit; i++) {
            ...
            if (UpdatedCount <= 3) {
                ans += solver(pos + 1, UpdatedCount, bound & (i == s.charAt(pos) - '0'));
            }
        }

        dp[pos][count][(bound == true) ? 1 : 0] = ans;
        return ans;
    }
