...
public class boxStacking {
    ...
    static void solve() {
        ...
        for (int i = 0; i < n; i++) {
            arr[idx][0] = h[i];
            arr[idx][1] = Math.min(w[i], l[i]);
            arr[idx][2] = Math.max(w[i], l[i]);
            idx++;

            arr[idx][0] = w[i];
            arr[idx][1] = Math.min(h[i], l[i]);
            arr[idx][2] = Math.max(h[i], l[i]);
            idx++;

            arr[idx][0] = l[i];
            arr[idx][1] = Math.min(w[i], h[i]);
            arr[idx][2] = Math.max(w[i], h[i]);
            idx++;
        }
